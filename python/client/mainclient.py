...
class MainClient:
    def __init__(self, access_token, refresh_token, timeout, progress_bar) -> None:
        ...
        try:
            ...
            logger.warning('SpotifyPlayer failed with stored cookie, trying browser fallback: ', exc_info=e)
            try:
                self.spotifyplayer = SpotifyPlayer()
            except Exception as e2:
                logger.error('SpotifyPlayer failed to create: ', exc_info=e2)
        ...
        add_event_listeners()
        try:
            self.client.connect(REGULAR_BASE, headers={'authorization': f'Bearer {access_token}', 'version': VERSION},
                                namespaces=['/api/authorization'])
            self.client.start_background_task(self.client.wait)
            self.client.start_background_task(self.check_logs)
