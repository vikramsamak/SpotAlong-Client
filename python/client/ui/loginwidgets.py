...
class LoginUi(QtWidgets.QMainWindow):
    ...
    def __init__(self, starting, user_func, *args, **kwargs):
        ...
        self.label_3.setText('A browser window will open to log in with Spotify.\nAfter logging in, enter the code below:')
        ...
        def callback(auth_url, _):
            ...
            if not opened:
                self.label_2.setText('Could not open a browser. Please open the URL manually.')
                self.pushButton.setDisabled(False)
                emitter = []
                return
            self.pushButton.setDisabled(False)
